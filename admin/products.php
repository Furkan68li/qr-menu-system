        ...
        <table class="w-full text-left border-collapse">
            <thead>
                <tr class="border-b border-white/5 bg-white/5">
                    <th class="px-6 py-4 text-xs font-semibold text-gray-400 uppercase tracking-wider">Görsel</th>
                    <th class="px-6 py-4 text-xs font-semibold text-gray-400 uppercase tracking-wider">Ürün Bilgisi</th>
                    <th class="px-6 py-4 text-xs font-semibold text-gray-400 uppercase tracking-wider">Kategori</th>
                    <th class="px-6 py-4 text-xs font-semibold text-gray-400 uppercase tracking-wider">Fiyat</th>
                    <th class="px-6 py-4 text-xs font-semibold text-gray-400 uppercase tracking-wider">Durum</th>
            ...
            <tbody class="divide-y divide-white/5">
                <?php foreach ($urunler as $urun): ?>
                    <tr class="group hover:bg-white/5 transition-colors">
                        <td class="px-6 py-4">
                            <div class="w-12 h-12 rounded-lg overflow-hidden bg-white/5 border border-white/10 relative">
                        ...
                        <td class="px-6 py-4">
                            <div class="text-white font-medium mb-0.5"><?= htmlspecialchars($urun['urun_adi']) ?></div>
                            <div class="text-xs text-gray-500 truncate max-w-[200px]">
                                <?= htmlspecialchars($urun['aciklama']) ?></div>
                        </td>
                        <td class="px-6 py-4">
                            <span
                                class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-purple-500/10 text-purple-400 border border-purple-500/10">
                                <?= htmlspecialchars($urun['kategori_adi']) ?>
                            </span>
                        </td>
                        <td class="px-6 py-4 font-semibold text-white">
                            <?= number_format($urun['fiyat'], 2) ?> <span class="text-xs text-gray-500">₺</span>
                        </td>
                        <td class="px-6 py-4">
                            <?php if ($urun['aktif']): ?>
                                <span
                                    class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-emerald-500/10 text-emerald-400 border border-emerald-500/10">
                                    <span class="w-1.5 h-1.5 rounded-full bg-emerald-500 mr-1.5"></span> Aktif
                                </span>
                            <?php else: ?>
                                <span
                                    class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-gray-500/10 text-gray-400 border border-gray-500/10">
                                    <span class="w-1.5 h-1.5 rounded-full bg-gray-500 mr-1.5"></span> Pasif
                                </span>
                            <?php endif; ?>
                        </td>
                    ...
                    <tr>
                        <td colspan="6" class="px-6 py-12 text-center text-gray-500">
                            <div class="flex flex-col items-center justify-center">
                                ...
                                <p class="text-lg font-medium text-gray-400">Henüz ürün bulunmuyor</p>
                                <p class="text-sm mt-1">Yeni bir ürün ekleyerek başlayın.</p>
                            </div>
                        </td>
                    </tr>
                <?php endif; ?>
